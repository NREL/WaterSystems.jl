...
function junction_to_struct(data::Dict{Int64,Any})
    junctions = [Junction(j["number"], j["name"], j["elevation"], j["head"], j["demand"], j["demandforcast"], j["minimum_pressure"], j["coordinates"]) for (key,j) in data]
    return junctions
end
function node_to_struct(data::Dict{String, Any})
    nodes = [Junction(j["number"], j["name"], j["elevation"], j["head"], j["demand"], j["demandforcast"], j["minimum_pressure"], j["coordinates"]) for (key,j) in data]
    return nodes
end
...
function tank_to_struct(data::Dict{Int64,Any})
    ...
    for (key, t) in data
        node = t["node"]
        junction = Junction(node["number"], node["name"], node["elevation"], node["head"], node["demand"], node["demandforcast"], node["minimum_pressure"], node["coordinates"])
...
function pipe_to_struct(data::Dict{Int64,Any})
    ...
    for (key, p) in data
        ...
        junction_from = Junction(j_from["number"], j_from["name"], j_from["elevation"], j_from["head"], j_from["demand"], j_from["demandforcast"], j_from["minimum_pressure"], j_from["coordinates"])
        junction_to = Junction(j_to["number"], j_to["name"], j_to["elevation"], j_to["head"], j_to["demand"], j_to["demandforcast"], j_to["minimum_pressure"], j_to["coordinates"])
...
function demand_to_struct(data::Dict{Int64,Any})
    ...
    for (key, d) in data
        node_data = d["node"]
        node = Junction(node_data["number"], node_data["name"], node_data["elevation"], node_data["head"], node_data["demand"], node_data["demandforcast"], node_data["minimum_pressure"], node_data["coordinates"])
        push!(demands, WaterDemand(d["name"], node , d["status"], d["max_demand"], d["demand"]))
    end
    return demands
end
