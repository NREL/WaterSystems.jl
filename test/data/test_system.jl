...
storages = [ RoundTank("T1", junctions[4], (min = 0.0, max = 2500.0), 10.0, 500.0, pi*25, 200/(10*pi), (min=0.0, max=1000/(10*pi)))];

sources = [SourceReservoir("R1",junctions[1],0)];
...
pipes = [   ReversibleFlowPipe(1,"P1", (from=junctions[4],to=junctions[3]), 0.6, 100.0, 200.0, 2.0, 10.0, 1),
            StandardPositiveFlowPipe(2,"P2",(from=junctions[2],to=junctions[2]),0.6,150.0,200.0,5.0,30.0,1)
        ];

valves = [  PressureReducingValve(1, "V1", (from=junctions[1],to=junctions[2]), "open", 1.0, 1.0)
        ];

elec_price = rand(24)

pumps = [ConstSpeedPump(1,"PMP1",(from=junctions[5],to=junctions[4]), 1,
...
system = WaterSystem(junctions, storages, sources, pipes, valves, pumps, demands);
